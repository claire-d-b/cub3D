...
float raycast(t_player *player, char **map, float angle)
{
	...
	while (p != '1')
	{
		...
		if (player->struct_side.he == player->struct_side.he_tmp && player->struct_side.wi >= player->struct_side.wi_tmp && player->struct_side.west == 0 && player->struct_side.north == 0 && player->struct_side.south == 0)
		{
			init_struct_side(player);
			player->struct_side.east = 1;
		}
		else if (player->struct_side.wi % player->y == 0)
		{
			init_struct_side(player);
			player->struct_side.west = 1;
		}
		else if (player->struct_side.wi % player->x)
		{
			init_struct_side(player);
			player->struct_side.south = 1;
		} 
		else if (player->struct_side.he == player->struct_side.he_tmp && player->struct_side.wi <= player->struct_side.wi_tmp)
		//else
		{
			init_struct_side(player);
			player->struct_side.west = 1;
		}
		else if (player->struct_side.wi == player->struct_side.wi_tmp && player->struct_side.he <= player->struct_side.he_tmp)
		//else
		{
			init_struct_side(player);
			player->struct_side.south = 1;
		...
		player->struct_side.distance = (int)(d * cos (fabs(angle - player->teta)));
