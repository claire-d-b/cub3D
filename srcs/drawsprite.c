...
void	register_sprite_end(int i, t_player *player, float angle, float d)
{
	...
	player->sprite[i][3] =
	player->struct_screen.i;
	player->sprite[i][13] = (player->sprite[i][13] == 0 && player->dist_wall > d * cos(fabs(angle - player->teta))) ?
	player->struct_screen.i : player->sprite[i][13];
	player->sprite[i][12] = (player->dist_wall > d * cos(fabs(angle - player->teta))) ?
...
void	draw_sprite_from_start(t_player *player, int i, int j, int count)
{
	...
	if (
	color > 0 && xposition_start + i <= player->sprite[count][12]
	...
	if (
	color > 0 && xposition_end - i <= player->sprite[count][12]
