...
/*   Updated: 2020/09/20 13:48:06 by clde-ber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	player->sprite[i][0] = (player->sprite[i][0] == 0 && get_decimals(player->ray_y + d * cos(angle)) > 48 && 
	get_decimals(player->ray_y + d * cos(angle)) < 52) ?
	player->ray_x + d * sin(angle) : player->sprite[i][0];
	...
	player->sprite[i][1] = (player->sprite[i][1] == 0 && get_decimals(player->ray_x + d * sin(angle)) > 48 && 
	get_decimals(player->ray_x + d * sin(angle)) < 52) ?
	player->ray_y + d * cos(angle) : player->sprite[i][1];
...
void	draw_sprite_from_start(t_player *player, int i, int j, int count)
{
	...
	if (j <= wall_h && (((get_decimals(player->sprite[count][1]) * wall_h / 100)) + i) <= wall_h &&
	(((get_decimals(player->sprite[count][1]) * wall_h / 100)) + i) >= 0 && j >= 0)
	{
		color = set_texture_sprite(player, j * player->ids.xpm_sprite_h /
		wall_h, (((get_decimals(player->sprite[count][1]) * wall_h / 100)) + i) * player->ids.xpm_sprite_w / wall_h);
	}
	if (
