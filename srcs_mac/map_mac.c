...
/*   Updated: 2020/10/14 11:32:40 by clde-ber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		check_surr_walls(char **map, int i, int j)
{
	...
	(int)ft_strlen(map[i])) > 0)
	{
		while (j + x < (int)ft_strlen(map[i - 1]))
		{
			if (map[i - 1][j + x] != '1' && is_space(map[i - 1][j + x]) == 0)
				return (1);
			x++;
		}
	}
	...
	(int)ft_strlen(map[i])) < 0)
	{
		while (j + x < (int)ft_strlen(map[i]))
		{
			if (map[i][j + x] != '1' && is_space(map[i][j + x]) == 0)
				return (1);
			x++;
		}
	}
...
void	check_map(char **map, int count, t_player *player)
{
	...
	while (map[++i])
	{
		if (i && ((((int)ft_strlen(map[i - 1]) -
		(int)ft_strlen(map[i])) == 0 && (j = (int)ft_strlen(
		map[i]) - 1) && j > 0 && (map[i - 1][j] != '1' || map[i][j] != '1'))
		|| (((int)ft_strlen(map[i - 1]) - (int)ft_strlen(
		map[i])) > 0 && (j = (int)ft_strlen(map[i])) && j > 0 &&
		check_surr_walls(map, i, j)) || (((int)ft_strlen(map[i - 1])
		- (int)ft_strlen(map[i])) < 0 && (j = (int)ft_strlen(
