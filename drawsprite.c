...
void	draw_sprite_from_end(t_player *player, int i, int j, int count)
{
	...
	i >= 0 && j >= 0)
	{
	color = set_texture_sprite(player, j * player->ids.xpm_sprite_h / wall_h,
	player->ids.xpm_sprite_w - (i * player->ids.xpm_sprite_w / wall_h));
...
void	draw_sprite_from_start(t_player *player, int i, int j, int count)
{
	...
	{
	color = set_texture_sprite(player, j * player->ids.xpm_sprite_h / wall_h,
	i * player->ids.xpm_sprite_w / wall_h);
...
/*	int x;
	...
	x = 0;
	y = 0;
	while (player->sprite[x] && player->sprite[x][y] != 0)
	{
		while (y < 10)
		{
			printf("%f\n", player->sprite[x][y]);
